...
---------------------------------BOSS_NAME도 같이
  ...
  CREATE OR REPLACE VIEW VEMP_BOSS AS
SELECT T1.*, T2.EMP_NAME BOSS_NAME
    FROM( SELECT
        A.EMP_ID, A.EMP_NAME, A.DEPT_CODE,B.DEPT_NAME, B.PARENT_DEPT,
            DECODE(B.BOSS_ID, A.EMP_ID, C.BOSS_ID, B.BOSS_ID) BOSS_ID, A.SALARY
